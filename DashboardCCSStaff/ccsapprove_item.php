...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $queryUpdateItemStatus = "UPDATE tblborrowingreports SET itemreqstatus = 'Approved', approvebyid = ?, datimeapproved = ?  WHERE id = ? AND itemreqstatus = 'Pending Borrow'";
    $stmtUpdateItemStatus = mysqli_prepare($con, $queryUpdateItemStatus);

    if ($stmtUpdateItemStatus) {
        mysqli_stmt_bind_param($stmtUpdateItemStatus, "isi", $approvebyid, $datimeapproved, $itemId);
        ...
        if (mysqli_stmt_execute($stmtUpdateItemStatus)) {
            ...
            if ($stmtUpdateItemBrandStatus) {
                ...
                if (mysqli_stmt_execute($stmtUpdateItemBrandStatus)) {
                    echo "<script>window.location.href='ccsstaffViewBorrower_all_items.php?msg_success=Items approved successfully';</script>";
                } else {
                    // Log the error instead of displaying to users
