...
$query = "SELECT * FROM `tblitembrand` WHERE `status` = 'Borrowed' ORDER BY categoryname, subcategoryname";
...
<!-- Display the list of borrowed items as cards -->
<div class="container">
    <h2><i class='fas fa-handshake me-2'></i>List of Borrowed Items</h2>
    <div class="row row-cols-1 row-cols-md-1 row-cols-lg-4 g-2">
    <?php
while ($rowItem = mysqli_fetch_assoc($result)) {
    // Fetch the last borrowed item details for the current item
    ...
    if ($stmtSelect) {
        ...
        if ($resultSelect) {
            ...
            if ($row) {
                ...
                echo '</div>';
                echo '<div class="mb-3">';
               // Assuming $row['datetimeborrow'] contains the date and time in the format 'Y-m-d H:i:s'
                $dateBorrowed = new DateTime($row['datetimeborrow'], new DateTimeZone('UTC'));
                $dateBorrowed->setTimezone(new DateTimeZone('Asia/Manila'));
                echo '<h7 class=" card-text">Date Borrowed: ' . $dateBorrowed->format('m-d-Y') . '</h7>';
                echo '</div>';
                echo '<div class="mb-2 text-start">';
                echo '<a href="ccsstaffReturnItem.php?item_id=' . $rowItem['id'] . '" class="btn btn-primary">Return Item</a>';
                echo '</div>';
                echo '</div>';
                echo '</div>';
                echo '</div>';
