...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = mysqli_prepare($conn, $insert_message_query);
    if ($stmt) {
        mysqli_stmt_bind_param($stmt, "is", $adminId, $message);
        if (mysqli_stmt_execute($stmt)) {
            ...
            if ($recipient_result && mysqli_num_rows($recipient_result) > 0) {
                while ($recipient_row = mysqli_fetch_assoc($recipient_result)) {
                    ...
                    $stmt_recipient = mysqli_prepare($conn, $insert_recipient_query);
                    if ($stmt_recipient) {
                        mysqli_stmt_bind_param($stmt_recipient, "ii", $message_id, $recipient_id);
                        mysqli_stmt_execute($stmt_recipient);
                    } else {
                        echo "Error inserting recipient: " . mysqli_error($conn);
                    }
                }
            ...
            header('Location: adminMessaging.php?msg_success=Message sent successfully');
            exit();
        } else {
            // Redirect with error message
            header('Location: adminMessaging.php?msg_fail=Failed to send message');
            exit();
        }
    } else {
...
$stmt_conversation = mysqli_prepare($conn, $conversation_query);
if ($stmt_conversation) {
    mysqli_stmt_bind_param($stmt_conversation, "ii", $adminId, $adminId);
    if (mysqli_stmt_execute($stmt_conversation)) {
        $conversation_result = mysqli_stmt_get_result($stmt_conversation);
    } else {
        echo "Error executing conversation query: " . mysqli_error($conn);
    }
} else {
                        ...
                        <div class="message-box" id="message-box">
                            ...
                            if (isset($conversation_result) && mysqli_num_rows($conversation_result) > 0) {
                                while ($conversation_row = mysqli_fetch_assoc($conversation_result)) {
                                    if ($conversation_row['sender_id'] == $adminId) {
                                        // admin message
